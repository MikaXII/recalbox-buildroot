...
#SUPER_MARIO_WAR_CONF_OPTS = -DDISABLE_DEFAULT_CFLAGS=1
...
define SUPER_MARIO_WAR_CONFIGURE_CMDS
	mkdir $(@D)/build && \
	cd $(@D)/build && \
	cmake .. -DDISABLE_DEFAULT_CFLAGS=1 -DCMAKE_C_FLAGS="$(TARGET_CFLAGS)" -DCMAKE_CXX_FLAGS="$(TARGET_CXXFLAGS)"
endef

define SUPER_MARIO_WAR_BUILD_CMDS
	$(MAKE) CFLAGS="$(TARGET_CFLAGS)" CXXFLAGS="$(TARGET_CXXFLAGS)" CXX="$(TARGET_CXX)" CC="$(TARGET_CC)" -C $(@D)/build 
endef
